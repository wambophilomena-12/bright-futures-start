    ...
    >
      ...
      >
        ...
        {(type === "TRIP" || type === "EVENT") && (
          <Badge className={cn("absolute top-1.5 left-1.5 backdrop-blur text-[10px] md:text-xs z-10 px-1.5 py-0.5 md:px-2 md:py-1 lowercase", tealBgClass)}>
            {type.toLowerCase()}
          </Badge>
        )}

        {type !== "EVENT" && type !== "TRIP" && showBadge && (
          <Badge className={cn("absolute top-1.5 left-1.5 backdrop-blur text-[8px] md:text-[0.6rem] z-10 px-1 py-0.5 md:p-1 lowercase", tealBgClass)}>
            {type.toLowerCase()}
          </Badge>
        )}

        {onSave && (
          <Button size="icon" onClick={handleSaveClick} className="absolute top-1.5 right-1.5 z-20 h-8 w-8 bg-transparent hover:bg-transparent border-none shadow-none outline-none">
        ...
        {isTripOrEvent && !minimalDisplay && (
          <div className="flex items-center justify-between gap-1 pt-1 border-t border-border/50 mt-auto"> 
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isCustomDate ? (
                <span className="text-[10px] md:text-xs text-muted-foreground italic">Flexible Date</span>
              ) : (
